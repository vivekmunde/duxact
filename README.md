...
`duxact` uses [**deep comparison**](https://github.com/vivekmunde/duxact/tree/1.x#deep-comparison) to detect changes in the selected state and it triggers rerendering only if the selected state has changed.
...
`connect` & `connectState` accepts a comparison function. This function receives oldState and newState. The function is expected to return boolean result, `false` indiates that the state has been changed. So returning `false` will trigger the rerendering.
